...
async function createStudentAccountForEnrollment(enrollmentId) {
  ...
  if (found.rows[0]) {
    ...
    await pool.query(
      'UPDATE student_accounts SET password_hash=$1,password_salt=$2,status=\'active\' WHERE student_id=$3',
    ...
    await pool.query(
      ...
      [hashSessionToken(activationToken), account.student_id, temporaryPassword, new Date(Date.now() + 30 * 60 * 1000).toISOString()]
    );
    return { account, temporaryPassword, activationToken };
  }
...
`;
  ...
  if (server.includes(callbackNeedle)) {
    server = server.replace(
      callbackNeedle,
      "const activation = await createStudentAccountForEnrollment(row.enrollment_id);\n      const activationToken = activation?.activationToken || '';\n      return res.redirect('/student-dashboard?activated=1&token=' + encodeURIComponent(activationToken));"
    );
  }
