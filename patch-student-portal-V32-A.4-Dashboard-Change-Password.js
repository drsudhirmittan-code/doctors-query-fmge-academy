...
if (!html.includes('DQ_V32_A4_DASHBOARD_CHANGE_PASSWORD')) {
  const ui = String.raw`
<!-- DQ_V32_A4_DASHBOARD_CHANGE_PASSWORD -->
<script>
(function(){
  async function installV32A4ChangePassword(){
    try{
      const dashboard = document.getElementById('dashboard');
      if(!dashboard) return false;
      if(document.getElementById('dq-v32-a4-dashboard-card')) return true;

      const response = await fetch('/api/student/me', { credentials:'include' });
      if(!response.ok) return false;

      const card = document.createElement('div');
      card.id = 'dq-v32-a4-dashboard-card';
      card.className = 'dq-v32-card';
      card.setAttribute('data-patch','DQ_V32_A4_DASHBOARD_CHANGE_PASSWORD');
      card.innerHTML =
        '<h3>Account Security</h3>' +
        '<p class="muted">Keep your Student Portal password secure.</p>' +
        '<button type="button" id="dq-v32-a4-change-btn">Change Password</button>';

      dashboard.appendChild(card);

      const button = document.getElementById('dq-v32-a4-change-btn');
      if(button){
        button.addEventListener('click', function(){
          const securityButton = document.getElementById('dq-v32-change-btn');
          if(securityButton){
  ...
  installV32A4ChangePassword();
  setTimeout(installV32A4ChangePassword, 300);
  setTimeout(installV32A4ChangePassword, 1000);
  setTimeout(installV32A4ChangePassword, 2000);
})();
</script>
`;
...
console.log('Applied Doctors Query FMGE Academy V32-A.4 Dashboard Change Password patch.');
